...
function Get-OverviewSummary {
    ...
    $content = (Read-TextFileWithRetry -Path $Path).Replace("`r`n", "`n").Trim()
    ...
    if ($headingMatch.Success) {
        return [pscustomobject]@{
            Summary = $body.Substring(0, $headingMatch.Index).Trim()
        }
    }
...
function Set-OrCheckMarkdown {
    ...
    $normalizedCurrent = if (Test-Path -LiteralPath $Path) {
        (Read-TextFileWithRetry -Path $Path).Replace("`r`n", "`n")
    }
    else {
        $null
    }
...
foreach ($entry in Get-HookOverviewEntries) {
    ...
    if (-not (Test-Path -LiteralPath $fullPath)) {
        Write-Warning "Skipping hook overview enrichment for missing page: $fullPath"
        continue
    }

    $overview = Get-OverviewSummary -Path $fullPath
    $content = Render-HookOverview `
        -Entry $entry `
