...
function Get-OverviewSummary {
    ...
    $logicalGroupCallout = (($body -split "`n") | Where-Object {
        $_ -match '^> Logical group: '
    } | Select-Object -First 1)

    # Keep the logical-group callout generated by the docs generator idempotent by
    # stripping it from the summary body before the overview template is re-rendered.
    ...
    if ($headingMatch.Success) {
        return [pscustomobject]@{
            ...
            LogicalGroupCallout = $logicalGroupCallout
        }
    }
...
function Render-HookOverview {
    ...
    if (-not [string]::IsNullOrWhiteSpace($LogicalGroupCallout)) {
        $sections += @(
            '',
            $LogicalGroupCallout.Trim()
        )
    }
...
foreach ($entry in Get-HookOverviewEntries) {
    ...
    $content = Render-HookOverview `
        ...
        -LogicalGroupCallout $overview.LogicalGroupCallout
