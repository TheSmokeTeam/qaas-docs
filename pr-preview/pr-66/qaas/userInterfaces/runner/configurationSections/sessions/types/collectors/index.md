...
Use this page for behavior and YAML shape. The same action can be built in C# with the [CollectorBuilder API](https://TheSmokeTeam.github.io/qaas-docs/qaas/functions/builders/collectors/index.md); that page is the function reference for the code-first surface.
...
Prometheus collection intervals

When collecting data from Prometheus between 2 date times the data is returned at the requested `SampleIntervalMs` interval. However, the times will be rounded by Prometheus to the closest past times divisible by the `SampleIntervalMs` value, which means a rounded time from before the start of the collection range will also be returned. For example, if the interval is 1 minute, the start time is `13:59:56` and the end time is `14:00:02`, the timestamps of the data samples returned will be `[13:59:00, 14:00:00]`.
