<!-- generated hash:b242ed71d61e sources:Runner, assert, cli-command -->
...
| ---- | -------- | ----------- | --------- | -------- | ------- | ---------- | ----------- |
| `--assertion-categories` | `AssertionCategoriesToRun` | `QaaS.Runner.Options.BaseOptions` | Yes | No | [] | `System.Collections.Generic.IList<System.String>` | Used to filter the assertion categories to run. The sessions that are associated with assertions will run too. |
| `-a`, `--assertion-names` | `AssertionNamesToRun` | `QaaS.Runner.Options.BaseOptions` | Yes | No | [] | `System.Collections.Generic.IList<System.String>` | Names of the assertions to run. Only sessions given to those assertions will run. |
...
| `--cases-name-patterns-ignore` | `CasesNamePatternsToIgnore` | `QaaS.Runner.Options.BaseOptions` | Yes | No | [] | `System.Collections.Generic.IList<System.String>` | Regex patterns of test-case names to ignore. |
| `-n`, `--cases-names` | `CasesNamesToRun` | `QaaS.Runner.Options.BaseOptions` | Yes | No | [] | `System.Collections.Generic.IList<System.String>` | Names of the test-cases to run. |
| `--cases-names-ignore` | `CasesNamesToIgnore` | `QaaS.Runner.Options.BaseOptions` | Yes | No | [] | `System.Collections.Generic.IList<System.String>` | Names of the test-cases to ignore. |
...
| `-e`, `--empty-results-directory` | `EmptyAllureDirectory` | `QaaS.Runner.Options.AssertableOptions` | Yes | No | False | `System.Boolean` | If flag is enabled will automatically empty the results directory before running. |
...
| `-l`, `--logger-level` | `LoggerLevel` | `QaaS.Framework.Executions.Options.LoggerOptions` | Yes | No |  | `System.Nullable<Serilog.Events.LogEventLevel>` | <br />The logger's level, overrides both the default logger's level (Information) and the level of any logger's configuration given.<br />All available options (not case sensitive) are: Verbose, Debug, <br />Information, Warning, Error, Fatal. |
...
| `-s`, `--serve-results` | `AutoServeTestResults` | `QaaS.Runner.Options.AssertableOptions` | Yes | No | False | `System.Boolean` | <br />If flag is enabled will automatically serve the test results in a human readable manner after performing the assertions. |
| `--session-categories` | `SessionCategoriesToRun` | `QaaS.Runner.Options.BaseOptions` | Yes | No | [] | `System.Collections.Generic.IList<System.String>` | Used to filter the session categories to run. |
| `-i`, `--session-names` | `SessionNamesToRun` | `QaaS.Runner.Options.BaseOptions` | Yes | No | [] | `System.Collections.Generic.IList<System.String>` | Names of the sessions to run. |
| `-w`, `--with-files` | `OverwriteFiles` | `QaaS.Runner.Options.BaseOptions` | Yes | No | [] | `System.Collections.Generic.IList<System.String>` | List of files to overwrite the qaas configuration with, The first file overwrites the qaas configuration file and then the one after it overwrite the result and so on... |
| `-f`, `--with-folders` | `OverwriteFolders` | `QaaS.Runner.Options.BaseOptions` | Yes | No | [] | `System.Collections.Generic.IList<System.String>` | List of folders whose yaml files overwrite the qaas configuration in alphabetical order, after overwrite files and in the order the folders are given. |
