<!-- generated hash:e8356ced0ff1 sources:Runner, assert, cli-command -->
...
| ---- | -------- | ----------- | --------- | -------- | ------- | ---------- | ----------- |
| `--assertion-categories` | `AssertionCategoriesToRun` | `Base options` | Yes | No | [] | `string list` | Used to filter the assertion categories to run. The sessions that are associated with assertions will run too. |
| `-a`, `--assertion-names` | `AssertionNamesToRun` | `Base options` | Yes | No | [] | `string list` | Names of the assertions to run. Only sessions given to those assertions will run. |
| `-c`, `--cases` | `CasesRootDirectory` | `Base options` | Yes | No |  | `string` | Name of the folder containing all the different .yaml case files that cases are ran with, If no folder is given will run the .qaas.yaml file as is without cases. |
| `--cases-name-patterns-ignore` | `CasesNamePatternsToIgnore` | `Base options` | Yes | No | [] | `string list` | Regex patterns of test-case names to ignore. |
| `-n`, `--cases-names` | `CasesNamesToRun` | `Base options` | Yes | No | [] | `string list` | Names of the test-cases to run. |
| `--cases-names-ignore` | `CasesNamesToIgnore` | `Base options` | Yes | No | [] | `string list` | Names of the test-cases to ignore. |
...
| `-e`, `--empty-results-directory` | `EmptyAllureDirectory` | `Assertable options` | Yes | No | False | `bool` | If flag is enabled will automatically empty the results directory before running. |
...
| `--no-env` | `DontResolveWithEnvironmentVariables` | `Base options` | Yes | No | False | `bool` | When this flag is used environment variables will not override loaded configurations. |
| `--no-process-exit` | `NoProcessExit` | `Base options` | Yes | No | False | `bool` | When this flag is used the runner will not terminate the current process after it completes. Useful when embedding QaaS.Runner and orchestrating multiple runners in a single host process. |
...
| `-s`, `--serve-results` | `AutoServeTestResults` | `Assertable options` | Yes | No | False | `bool` | If flag is enabled will automatically serve the test results in a human readable manner after performing the assertions. |
...
| `-f`, `--with-folders` | `OverwriteFolders` | `Base options` | Yes | No | [] | `string list` | List of folders whose yaml files overwrite the qaas configuration in alphabetical order, after overwrite files and in the order the folders are given. |
