...
| Results | `-s`, `--serve-results` | Yes | No | _empty_ | `string` | Serves Allure output after performing the assertions. Without a value the flag serves the default raw results folder `allure-results`. Provide a folder name such as `allure-report` to open a generated report directory, which is useful for Allure 3 flows. |
